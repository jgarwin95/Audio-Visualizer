...
void Container::Update() {
  for (Node& node : nodes_) {
    ...
    if (node.GetPos().x - radius >= rect_.x2 + 200 || node.GetPos().x + radius <= rect_.x1 - 200 ||
        node.GetPos().y - radius >= rect_.y2 + 200 || node.GetPos().y + radius <= rect_.y1 - 200) {
      ...
      if (side == 0) { // reset up
        node.ResetPosition(glm::vec2(x_pos, rect_.y1 - radius - 200));
        node.ResetVelocity(glm::vec2(pow(-1,direction)*x_velo,y_velo));
      } else if (side == 1) { // reset down
        node.ResetPosition(glm::vec2(x_pos, rect_.y2 + radius + 200));
        node.ResetVelocity(glm::vec2(pow(-1,direction)*x_velo,-y_velo));
      } else if (side == 2) { // reset right
        node.ResetPosition(glm::vec2(rect_.x2 + radius + 200, y_pos));
        node.ResetVelocity(glm::vec2(-x_velo,pow(-1,direction)*y_velo));
      } else if (side == 3) { // reset left
        node.ResetPosition(glm::vec2(rect_.x1 - radius - 200, y_pos));
        node.ResetVelocity(glm::vec2(x_velo,pow(-1,direction)*y_velo));
      }
    }
  }
